...
if __name__ == '__main__':
    ...
    log_dir = osp.join(os.path.abspath(model_output), 'ddpg')
    ...
    try:
        ...
        model = ddpg.load(checkpoint_file)
        model.set_env(env)
        ...
        model = ddpg(MlpPolicy, env, n_steps=nsteps, verbose=1)
        #model.full_pretrain(np.vstack([bc_train, bc_val]), batch_size=nsteps, n_epochs=500)
        model.pretrain(data_tr=bc_train, data_val=bc_val, batch_size=nsteps, n_epochs=5000)
