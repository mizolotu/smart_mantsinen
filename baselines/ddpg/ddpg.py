...
from common.base_vec_env import VecEnv
                ...
                while True:
                    for _ in range(log_interval):
                        ...
                        if self.eval_env is not None:
                            ...
                            self.eval_ep_info_buf.append({'r': np.mean(eval_rewards)})
                    ...
                    if self.eval_env is not None:
                        ...
                        if len(self.eval_ep_info_buf) > 0 and len(self.eval_ep_info_buf[0]) > 0:
                            logger.logkv('eval_ep_reward_mean', np.mean([ep_info['r'] for ep_info in self.eval_ep_info_buf]))
